...
/-- pa Enc Gen (B1 A1) (B2 A2') equals the guessing probability in Lemma 3.1.
    The semantic security game with (B1, B2) is exactly the guessing game where
    b is chosen uniformly, the b-th message is encrypted, and A2' tries to guess b
    from the ciphertext.
    The hypothesis hne (m0 ≠ m1 for all support points of A1) is essential:
    it ensures that R(![m0,m1][b], a) simplifies to (a == b),
    which fails when m0 = m1 since R would then accept both outputs. -/
lemma pa_eq_guessing
  ...
  -- Use hne to eliminate the if-terms arising from R(![m0,m1][b], a):
  -- when m0 ≠ m1 (guaranteed on the support of A1), R(m0, a) = [a=0] and R(m1, a) = [a=1],
  -- so the cross-terms (if m0=m1 ...) vanish.
  have key : ∀ a : M × M × St, A1 a *
    ...
    · have hne' := hne m0 m1 st hA1
      simp [hne', hne'.symm]
  simp_rw [key]
  -- Distribute 2⁻¹ out of the tsum using linearity
  simp_rw [mul_add, mul_left_comm (A1 _) 2⁻¹, ENNReal.tsum_add, ENNReal.tsum_mul_left]

/-- If |p0 - p1| > ε (real-valued, ε : NNReal) and hne holds for A1,
    then there exists B2' such that pa(Enc, Gen, B1(A1), B2(B2')) > 1/2 * (1 + ε).
    The witness B2' is A2 (case 1) or flipA A2 (case 2) from the Guessing Lemma.
    Proof:
      ...
      E. Rewrite pa via pa_eq_guessing (passing hne) and conclude. -/
lemma pa_gt_half
    (Enc : K → M → C) (Gen : PMF K)
    (A1 : PMF (M × M × St)) (A2 : C → St → PMF Bit)
    (ε : NNReal)
...
/-- For any simulator S, ps(B1(A1), S) ≤ 1/2,
    provided A1 only outputs distinct plaintext pairs (m0 ≠ m1 on the support).
    Proof: ps = 1/2 exactly. S receives only h(m) = 0 (no ciphertext info),
    so its success probability equals 1/2 for each support point by inner_eq_half's
    calculation, and ps is therefore the weighted average 1/2. -/
lemma ps_le_half
    (A1 : PMF (M × M × St))
    (S : Bit → St → PMF Bit)
    (hne : ∀ m0 m1 st, A1 (m0, m1, st) ≠ 0 → m0 ≠ m1) :
...
theorem semantic_security_implies_indistinguishable
  ...
  -- hne comes directly from the Indistinguishable definition;
  -- hne' converts it to the ≠ 0 form required by pa_gt_half and ps_le_half
  ...
  -- hss_ineq : |(pa Enc Gen (B1 A1) (B2 B2')).toReal - (ps (B1 A1) S).toReal| ≤ ((ε/2:NNReal):ℝ)

  -- Step 4: ps ≤ 1/2 for any simulator (hne' shared with Step 2)
  have hps : ps (B1 A1) S ≤ 1 / 2 := ps_le_half A1 S hne'

  -- Step 5: Derive contradiction via pa - ps > ε/2 vs hss_ineq ≤ ε/2.
  have hgap : (pa Enc Gen (B1 A1) (B2 B2')).toReal - (ps (B1 A1) S).toReal > (ε / 2 : ℝ) := by
    -- Convert hpa (ENNReal) to pa.toReal > (1/2*(1+ε)).toReal
  ...
  have habs : |(pa Enc Gen (B1 A1) (B2 B2')).toReal - (ps (B1 A1) S).toReal| > (ε / 2 : ℝ) := by
    ...
    rwa [abs_of_pos hpos]
  -- hss_ineq uses (ε/2 : NNReal) : ℝ; convert to (ε : ℝ)/2 for linarith
